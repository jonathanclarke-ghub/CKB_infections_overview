...
header <- data.frame(col_keys = col_names,
  #                   title   =rep(c("eTable 1: Antigens & pathogens included in multiplex serology panel with seropositivity definitions"),4),
                     head_1  = c("Pathogen","Antigen","Antigen seropositive above cutoff","Algorithm for pathogen seropositivity"),
                     head_2  = c("Pathogen","Antigen","Antigen seropositive above cutoff","Algorithm for pathogen seropositivity"))
...
ft<-flextable(tab, col_keys=c(col_names)) %>%
  ...
  merge_at(i=c(8:11),j=4,part="body") %>%
  merge_at(i=c(12:14),j=4,part="body") %>%
  merge_at(i=c(15:16),j=4,part="body") %>% 
  merge_at(i=c(19:20),j=4,part="body") %>%
  merge_at(i=c(21:22),j=4,part="body") %>%
  merge_at(i=c(24:25),j=4,part="body") %>%
  merge_at(i=c(26:27),j=4,part="body") %>%
  merge_at(i=c(43:50),j=4,part="body") %>%
  merge_at(i=c(52:53),j=4,part="body") %>%
...
save_as_docx(ft,path="K:/kadoorie/Staff_Folders/JonathanC/Projects/Serology/Tables_figures/Tables_figures_10/eTable_1t.docx",
